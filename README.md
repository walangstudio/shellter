...
Set `CLAUDE_HOOK_LOG=/path/to/log.jsonl` to record one JSON line per decision (timestamp, hook, tool, decision, reason, snippet). Off by default, fail-silent on write errors.
...
**False positive on a deny**: Set `CLAUDE_HOOK_LOG=/tmp/hook.log`, reproduce the issue, then check the log for the matched reason. Adjust the regex (or add a project-specific allow rule).
