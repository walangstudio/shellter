...
Both must be green. CI runs both on ubuntu 18/20/22/24, windows 20 and macos 20.
...
- The bash `rm` path (rmDanger/rmTargetDanger) does NOT cover the Windows/PowerShell delete verbs: `rm` is matched by name, and RM_SYSTEM_PREFIX is leading-slash-only so it never matches `C:\Windows`. `Remove-Item`/`ri`/`del`/`erase`/`rd`/`rmdir` need their own classifier (`winDeleteTargetDanger`/`winDeleteDanger`). The old coarse regexes only matched drive-root/`$HOME`/wildcard AND false-positived on a `$env:USERPROFILE\subdir` cleanup (the var name matched anywhere in the arg) - a target that needs an own-home carve-out cannot be done as a substring regex, tokenize it. Reason destructive-op coverage for BOTH the Unix and Windows verb families.
- An approve-list ALIAS word matches a cmdlet through the `-` word boundary: `clear` (screen-clear) in POSH_APPROVE matched `Clear-Content`/`Clear-Item`/`Clear-RecycleBin` (`\bclear\b` fires before the `-`), auto-approving a file truncate. Gate an alias that is also a cmdlet prefix with `(?!-)`.
- A path-target classifier MUST canonicalize before matching: resolve `.`/`..` (path.win32.normalize), fold `/`->`\`, strip quotes/backticks/`{(...)}` block wrappers/`\\?\`+`FileSystem::` prefixes, expand `~`/`$HOME`/`$env:*`, and strip trailing dots/spaces. Substring regexes miss `C:\tmp\..\Windows`, `${env:X}`, `C:\Windows.`, a glued `{Remove-Item ... C:\Windows}`, and forward slashes. But the trailing-dot/space strip must NOT eat a `.`/`..` SEGMENT (`/([^\\.\s])[. ]+(?=\\|$)/` keeps the preceding char) or it turns `\..\` into `\\` and the escape resolves wrong.
- A command/delete verb predicate that runs on BOTH tools must match the verb only at COMMAND position (stage start, opening a `{`/`(` block, after `&`/`.`), never at any token: scanning all positions made `grep del -r dir` and `grep -n rd -r x` (bash) hard-deny. Replacing an existing rule must preserve every case it covered - the rewrite silently dropped the home-root and `home\*` denies the two old regexes had. Fable's DO-NOT-SHIP round caught all of these; verify a replacement against the OLD rule's cases side by side.
- PRE-EXISTING (not yet fixed): the PowerShell approve path (`checkSegmentApprove` isPosh branch) does NOT split pipe stages, so `Get-ChildItem C:\Windows | Remove-Item -Recurse -Force` and `Get-Date | Set-Content ~\.ssh\authorized_keys` AUTO-APPROVE. The bash path splits stages; the PS branch must too (require every stage to approve). Deferred to its own PR (approve-path FP risk).
- A canonicalizer that can return `''` (or a bare drive) will hand a CATASTROPHIC verdict to junk: a lone `}`/`)` token from a braced block canonicalized to `''`, the drive-root regex `/^(?:[A-Za-z]:)?\\*$/` matched `''`, and `try { Remove-Item .\dist } catch {}` hard-denied (critical FP on routine code). Guard `if (!t) return null` and make every root/anchor regex reject empty. Also: a trailing-dot/space strip must keep the char before the dots (`/([^\\.\s])[. ]+/`) or it eats a `..` segment; a `-Recurse:<val>` test must treat any value except `$false`/`0` as recursive (`-Recurse:2` recurses); a Users/system container check must be STATIC (`C:\Users`), not derived from the running user's home, or it flips between Windows and Linux CI.
- A command/verb can be GLUED with no space after a boundary char (`if($true){Remove-Item`, `1|%{ri`). Take the run after the last `{`/`(` in the token as the command candidate, and count a spaced `{` prev too, so `ForEach-Object { Remove-Item … $HOME }` denies instead of falling to the approve pass and auto-approving a home wipe. Do NOT extend the boundary/prev set to `=`/`@`/`;` — that false-positived on `rsync --exclude=rd`, `[ "$x" = del ]`, and splat idioms (a delete verb appearing as an option value or after a bash `=`). And bound the arg scan to the verb's OWN block (track `{(`/`)}` depth, stop at the closing `}` and at else/catch/finally) or a sibling `catch { Get-ChildItem C:\Users\Public }` branch's path is read as a delete target (hard-deny FP), and a later `-WhatIf` in an `else` aborts the check (bypass).
- Windows/PS delete coverage is a rabbit hole: PowerShell is a full language and bash `tokenizeArgs` is wrong for it (quoting, splat, providers, param binding, script blocks), so a comprehensive static classifier keeps trading a bypass for a false-positive across review rounds. After 3 Fable rounds each finding routine-code FPs, the user chose a CONSERVATIVE core (2026-09-23): deny a PLAIN recursive delete of a system/home/drive/Users/registry target at command position via the canonicalizer; leave a delete hidden in an assignment/splat/comma-array/here-string/trailing-`\`-quote as a documented PROMPT (bypass), never a false deny. Prefer a documented bypass over an unappealable FP whenever the parse is ambiguous. The PS approve-path pipe/scriptblock hole, the `rm` alias on Windows targets, and git-bash `/c/` mounts are deferred to a follow-up.
- "Fall through" is NOT neutral in shellter: a deny predicate returning null hands the command to the approve pass, and a read-only-looking wrapper (`Get-ChildItem … | ForEach-Object { … }`) then AUTO-APPROVES it. Making computed (`$var`) delete targets fall through turned a HEAD-denied `ForEach-Object { Remove-Item -Recurse -Force "$HOME\$_" }` home wipe into a silent allow. Uncertain-but-dangerous => an `ask`-severity predicate (runs before approve), never plain fallthrough. Diff every relaxation against HEAD (`git show 0944569:hooks/check-bash.js > hooks/_head_check.js`, run both, delete) - the per-case review rounds missed this.
- Every deny/ask predicate runs on EVERY match variant, including the one where shellter expanded host vars (`$HOME` -> the real home). On a Unix host that variant starts with `/`, so a rule keyed on a leading `/` sees `/home/me$_` even though the user typed `"$HOME\$_"`: the macOS/Linux CI jobs hard-denied a case Windows asked on. Gate literal-only rules on "no `$` left in the target", and test PS home cases under a `/home/runner` and a `/Users/runner` env override, not just the Windows home.
